...
/*var creator;
var fullname;
   $.get("AjaxId.php", function(data, status){
                          creator = data;
                          });
    $.get("AjaxUid.php", function(data, status){
                          fullname = data;
                          });
*/
 var array="";
...
(function (factory) {
    ...
    var Comments = {
        ...
        events: {
            ...
            'click li.comment button.name': 'nameButtonClicked',
            'click li.comment button.follow': 'followButtonClicked',
        ...
        sortComments: function (comments, sortKey) {
            ...
            for(var y = 0; y<comments.length; y++){
                this.$el.find('[data-id="'+comments[y].id+'"]').show();
              }
            
            for(var i = 0; i<comments.length; i++){
            for(var x = 0; x<array.length; x++){
            if(comments[i].creator != array[x]){
            this.$el.find('[data-id="'+comments[i].id+'"]').show();
        }
    }
}
           ...
           //Sort by Followed people
              ...
              //  this.$el.find('[id="comment-list"]').hide();

            for(var y = 0; y<comments.length; y++){
                this.$el.find('[data-id="'+comments[y].id+'"]').hide();  
            }
        ...
        nameButtonClicked: function(ev) {
        var commentEl = $(ev.currentTarget).parents('li.comment').first();
            var commentModel = commentEl.data().model;

    $.post("Profile.php",
        {
          data: commentModel.creator,
          name: commentModel.fullname
        });

             window.location = 'Profile.php';
        },

        followButtonClicked: function(ev){
           var commentEl = $(ev.currentTarget).parents('li.comment').first();
            var commentModel = commentEl.data().model;
    ...
    $.post("Follow.php",
        ...
        createNavigationElement: function() {
            ...
            // Followed
             var followed = $('<li/>', {
                text: this.options.textFormatter(this.options.followText),
                'data-sort-key': 'follows',
                'data-container-name': 'comments'
            });
            ...
            dropdownNavigation.append(newest.clone()).append(oldest.clone());
             ...
             navigationWrapper.append(followed);
                dropdownNavigation.append(followed.clone());
        ...
        createCommentWrapperElement: function(commentModel) {
         ...
         /*   if(commentModel.profileURL) {
            ...
            if(commentModel.createdByCurrentUser || commentModel.createdByAdmin) nameLink.addClass('highlight-font-bold');
        ...
        createCommentJSON: function(textarea) {
            ...
            var commentJSON = {
                id: (this.getComments().length + 1),   // Temporary id
                parent: textarea.attr('data-parent') || null,
                created: time,
                modified: time,
                content: this.getTextareaContent(textarea),
